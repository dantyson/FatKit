(function(addon) {
    ...
    if (typeof define == "function" && define.amd) {
        define("fatkit-htmleditor", ["fatkit"], function(){
    ...
    UI.component('htmleditor', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('textarea[data-f-htmleditor]', context).each(function() {

                    var editor = UI.$(this), obj;

                    if (!editor.data('htmleditor')) {
                        obj = UI.htmleditor(editor, UI.Utils.options(editor.attr('data-f-htmleditor')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.content    = this.htmleditor.find('.f-htmleditor-content');
            this.toolbar    = this.htmleditor.find('.f-htmleditor-toolbar');
            this.preview    = this.htmleditor.find('.f-htmleditor-preview').children().eq(0);
            this.code       = this.htmleditor.find('.f-htmleditor-code');
            ...
            if (this.options.iframe) {
                ...
                this.iframe = UI.$('<iframe class="f-htmleditor-iframe" frameborder="0" scrolling="auto" height="100" width="100%"></iframe>');
            ...
            this.htmleditor.on('click', '.f-htmleditor-button-code, .f-htmleditor-button-preview', function(e) {

                e.preventDefault();

                if ($this.htmleditor.attr('data-mode') == 'tab') {

                    $this.htmleditor.find('.f-htmleditor-button-code, .f-htmleditor-button-preview').removeClass('f-active').filter(this).addClass('f-active');

                    $this.activetab = UI.$(this).hasClass('f-htmleditor-button-code') ? 'code' : 'preview';
            ...
            this.on('init.f.component', function() {
                $this.redraw();
            });

            this.element.attr('data-f-check-display', 1).on('display.f.check', function(e) {
        ...
        _buildtoolbar: function() {
            ...
            this.options.toolbar.forEach(function(button) {
                ...
                bar.push('<li><a data-htmleditor-button="'+button+'" title="'+title+'" data-f-tooltip>'+$this.buttons[button].label+'</a></li>');
            });
        ...
        fit: function() {
            ...
            if (mode == 'tab') {
                ...
                this.htmleditor.find('.f-htmleditor-button-code, .f-htmleditor-button-preview').removeClass('f-active')
                    .filter(this.activetab == 'code' ? '.f-htmleditor-button-code' : '.f-htmleditor-button-preview')
                    .addClass('f-active');
            }
    ...
    UI.components.htmleditor.template = [
        '<div class="f-htmleditor f-clearfix" data-mode="split">',
            '<div class="f-htmleditor-navbar">',
                '<ul class="f-htmleditor-navbar-nav f-htmleditor-toolbar"></ul>',
                '<div class="f-htmleditor-navbar-flip">',
                    '<ul class="f-htmleditor-navbar-nav">',
                        '<li class="f-htmleditor-button-code"><a>{:lblCodeview}</a></li>',
                        '<li class="f-htmleditor-button-preview"><a>{:lblPreview}</a></li>',
                        '<li><a data-htmleditor-button="fullscreen"><i class="fa fa-expand"></i></a></li>',
                    '</ul>',
                '</div>',
            '</div>',
            '<div class="f-htmleditor-content">',
                '<div class="f-htmleditor-code"></div>',
                '<div class="f-htmleditor-preview"><div></div></div>',
            '</div>',
        '</div>'
    ].join('');
    ...
    UI.plugin('htmleditor', 'base', {
        ...
        init: function(editor) {
            ...
            editor.htmleditor.on('click', 'a[data-htmleditor-button="fullscreen"]', function() {
                editor.htmleditor.toggleClass('f-htmleditor-fullscreen');

                var wrap = editor.editor.getWrapperElement();

                if (editor.htmleditor.hasClass('f-htmleditor-fullscreen')) {
        ...
        init: function(editor) {
            ...
            UI.$.extend(editor, {
                ...
                disableMarkdown: function() {
                    ...
                    this.htmleditor.find('.f-htmleditor-button-code a').html(this.options.lblCodeview);
                    this.render();
                }
            ...
            function enableMarkdown() {
                ...
                editor.htmleditor.find('.f-htmleditor-button-code a').html(editor.options.lblMarkedview);
            }
