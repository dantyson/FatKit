...
(function(addon) {
	...
	if (typeof define == "function" && define.amd) {
		define("fatkit-nestable", ["fatkit"], function(){
	...
	UI.component('nestable', {
		...
		boot: function() {
			...
			UI.ready(function(context) {
				...
				UI.$("[data-f-nestable]", context).each(function(){

					var ele = UI.$(this);

					if(!ele.data("nestable")) {
						var plugin = UI.nestable(ele, UI.Utils.options(ele.attr("data-f-nestable")));
					}
				});
			});
		},
		...
		init: function() {
			...
			var onStartEvent = function(e) {
				...
				$this.trigger('start.f.nestable', [$this]);
			};
			...
			var onMoveEvent = function(e) {
				if ($this.dragEl) {
					...
					$this.trigger('move.f.nestable', [$this]);
				}
			};
		...
		setParent: function(li) {
			if (li.children(this.options.listNodeName).length) {
				li.addClass("f-parent");
			}
		},

		unsetParent: function(li) {
			li.removeClass('f-parent '+this.options.collapsedClass);
		...
		dragStop: function(e) {
			...
			if (this.element[0] !== root[0]) {
				...
				root.trigger('change.f.nestable',[el, "added", root, root.data('nestable')]);
				this.element.trigger('change.f.nestable', [el, "removed", this.element, this]);

			} else {
				this.element.trigger('change.f.nestable',[el, "moved", this.element, this]);
			}

			this.trigger('stop.f.nestable', [this, el]);
