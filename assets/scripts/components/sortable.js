...
(function(addon) {
	...
	if (typeof define == "function" && define.amd) {
		define("fatkit-sortable", ["fatkit"], function(){
	...
	UI.component('sortable', {
		...
		defaults: {
			...
			childClass       : 'f-sortable-item',
			placeholderClass : 'f-sortable-placeholder',
			overClass        : 'f-sortable-over',
			draggingClass    : 'f-sortable-dragged',
			dragMovingClass  : 'f-sortable-moving',
			baseClass        : 'f-sortable',
			dragCustomClass  : '',
			handleClass      : false,
			group            : false,
		...
		boot: function() {
			...
			UI.ready(function(context) {
				...
				UI.$("[data-f-sortable]", context).each(function(){

					var ele = UI.$(this);

					if(!ele.data("sortable")) {
						var plugin = UI.sortable(ele, UI.Utils.options(ele.attr("data-f-sortable")));
					}
				});
			});
		...
		dragStart: function(e, elem) {
			...
			delayIdle = {
				...
				'apply'   : function() {
					...
					$this.trigger('start.f.sortable', [$this, currentlyDraggingElement]);
		...
		dragEnd: function(e, elem) {
			...
			if (currentlyDraggingElement) {
				...
				this.trigger('stop.f.sortable', [this]);
			}
		...
		triggerChangeEvents: function() {
			...
			triggers.forEach(function (trigger, i) {
				trigger.el.trigger('change.f.sortable', [trigger.el, currentlyDraggingElement, trigger.mode]);
			});
		},
