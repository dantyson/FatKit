(function(addon) {
	...
	UI.component('timepicker', {
		...
		boot: function() {
			...
			// init code
			UI.$html.on("focus.timepicker.fatkit", "[data-f-timepicker]", function(e) {

				var ele = UI.$(this);

				if (!ele.data("timepicker")) {
					var obj = UI.timepicker(ele, UI.Utils.options(ele.attr("data-f-timepicker")));
		...
		init: function() {
			...
			this.options.template  = '<ul class="f-nav f-nav-autocomplete f-autocomplete-results">{{~items}}<li data-value="{{$item.value}}"><a>{{$item.value}}</a></li>{{/items}}</ul>';

			this.options.source = function(release) {
				release(times[$this.options.format] || times['12h']);
			};

			this.element.wrap('<div class="f-autocomplete"></div>');

			this.autocomplete = UI.autocomplete(this.element.parent(), this.options);
			this.autocomplete.dropdown.addClass('f-dropdown-small f-dropdown-scrollable');

			this.autocomplete.on('show.f.autocomplete', function() {
